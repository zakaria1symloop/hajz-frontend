...
import { HiOutlineHome, HiOutlineLogout, HiOutlinePlus, HiOutlineOfficeBuilding, HiOutlineCurrencyDollar, HiOutlineClipboardList, HiOutlineCog, HiOutlineExclamationCircle, HiOutlineGlobeAlt } from 'react-icons/hi';
import { FaHotel, FaUtensils, FaCar } from 'react-icons/fa';
import Cookies from 'js-cookie';

const businessConfig: Record<BusinessType, { nameKey: string; icon: any; color: string; bgColor: string }> = {
  hotel: { nameKey: 'hotel', icon: FaHotel, color: 'text-blue-600', bgColor: 'bg-blue-50' },
  restaurant: { nameKey: 'restaurant', icon: FaUtensils, color: 'text-orange-600', bgColor: 'bg-orange-50' },
  car_rental: { nameKey: 'carRental', icon: FaCar, color: 'text-green-600', bgColor: 'bg-green-50' },
};
...
export default function ProDashboard() {
  ...
  const changeLanguage = (newLocale: string) => {
    // Set the locale cookie (used by next-intl)
  ...
  const businessName = t(config.nameKey);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className="fixed start-0 top-0 h-full w-64 bg-white border-e border-gray-200 z-40">
        {/* Logo */}
        <div className="h-16 flex items-center px-6 border-b border-gray-100">
          <Link href="/pro/dashboard" className="flex items-center gap-3">
            ...
            <span className="font-bold text-lg text-gray-900">{t('hajzPro')}</span>
          </Link>
        </div>
        ...
        <nav className="p-4 space-y-1">
          ...
          <div className="relative pt-2">
            <button
              ...
              className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 transition-colors w-full"
            >
              <HiOutlineGlobeAlt size={20} />
              {t('language')}
            </button>
            {langMenuOpen && (
              <div className="absolute start-0 bottom-full mb-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-50">
                <button
                  onClick={() => changeLanguage('en')}
                  className={`w-full px-4 py-2.5 text-start hover:bg-gray-50 transition-colors ${locale === 'en' ? 'bg-gray-50 font-medium' : ''}`}
                >
                  {t('english')}
                </button>
                <button
                  onClick={() => changeLanguage('fr')}
                  className={`w-full px-4 py-2.5 text-start hover:bg-gray-50 transition-colors ${locale === 'fr' ? 'bg-gray-50 font-medium' : ''}`}
                >
                  {t('french')}
                </button>
                <button
                  onClick={() => changeLanguage('ar')}
                  className={`w-full px-4 py-2.5 text-start hover:bg-gray-50 transition-colors ${locale === 'ar' ? 'bg-gray-50 font-medium' : ''}`}
                >
                  {t('arabic')}
                </button>
              </div>
      ...
      <main className="ms-64 p-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">{t('welcomeBack', { name: owner?.name?.split(' ')[0] || 'User' })}</h1>
          <p className="text-gray-500 mt-1">{t('manageYourBusiness', { business: businessName.toLowerCase() })}</p>
        </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-8">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-amber-800 mb-1">{t('completeProfile')}</h3>
                <p className="text-amber-700 text-sm mb-4">
                  {t('noBusinessYet', { business: businessName.toLowerCase() })}
                </p>
                <Link
                ...
                >
                  ...
                  {t('addYour', { business: businessName })}
                </Link>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  ...
                  {(business.status === 'verified' || business.verification_status === 'verified') ? t('verified') :
                   (business.status === 'pending' || business.verification_status === 'pending') ? t('pending') :
                   t('active')}
                </span>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {business ? (
                ...
                {businessType === 'hotel' && (
                  <>
                    ...
                    <Link href="/pro/hotel" className={`flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-xl hover:${config.bgColor} transition-colors group`}>
                      ...
                      <span className="text-sm font-medium">{t('editHotel')}</span>
                    </Link>
                    <Link href="/pro/reservations" className={`flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-xl hover:${config.bgColor} transition-colors group`}>
                      ...
                      <span className="text-sm font-medium">{t('viewBookings')}</span>
                    </Link>
                ...
                {businessType === 'restaurant' && (
                  <>
                    <Link href="/pro/plats" className="flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-orange-50 transition-colors group">
                      ...
                      <span className="text-sm font-medium">{t('manageMenu')}</span>
                    </Link>
                    <Link href="/pro/tables" className="flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-orange-50 transition-colors group">
                      ...
                      <span className="text-sm font-medium">{t('manageTables')}</span>
                    </Link>
                ...
                {businessType === 'car_rental' && (
                  <>
                    ...
                    <Link href="/pro/company" className="flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-green-50 transition-colors group">
                      ...
                      <span className="text-sm font-medium">{t('editCompany')}</span>
                    </Link>
              ...
              >
                ...
                <span className="font-medium">{t('addFirstBusiness', { business: businessName })}</span>
              </Link>
            )}
          </div>
        </div>
